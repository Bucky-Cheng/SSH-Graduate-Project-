			...
			<table class="table table-bordered">
	  		<thead>
                <tr>
                    ...
                    <th style="text-align:center">教师工号</th>
...
function remake(){
	jQuery.ajax({
					...
					var json=eval('('+data+')');
						for(var i in json){
						var tr="<tr><td style='text-align:center'>"+json[i].teacherID+"</td><td style='text-align:center'>"+json[i].teacherName+"</td>";
						var tr1=tr+"<td style='text-align:center'><button type='button' id="+json[i].teacherID+" class='btn btn-danger' onclick='del(this)'><span>退回</span> </button></td></tr>";
...
function del(delID){
	...
	var delID=jQuery(delID).attr("id"); 
	
	
	
	jQuery.ajax({
 	
					url:"backTExamineAction",
					type:"post",
					data:{delID:delID},
					dataType:  "json",
					success:function(data){
