...
public class serveTableDao {
	...
	public void setSessionFactory(SessionFactory sessionFactory) {
	 ...
	 public List<serveTable> getTableByName(String Name){
		 Session session=null;
		 try{
			 session=sessionFactory.openSession();
	 ...
	 public boolean updateEduTable(String name,String content){
			...
			try{
				...
				session=sessionFactory.openSession();
				//根据id获取要修改的用户数据
				String hql=("update serveTable e set e.content=? where name =?");  
				Query sqlquery = session.createQuery(hql);
	 ...
	 public boolean updateEduScore(String name,String Score){
			...
			try{
				...
				session=sessionFactory.openSession();
				//根据id获取要修改的用户数据
				String hql=("update serveTable e set e.score=? where name =?");  
				Query sqlquery = session.createQuery(hql);
	 ...
	 public boolean updateEduScoreA(String name,String Score){
			...
			try{
				...
				session=sessionFactory.openSession();
				//根据id获取要修改的用户数据
				String hql=("update serveTable e set e.AllScore=? where name =?");  
				Query sqlquery = session.createQuery(hql);
		...
		public int insertOneB(List<serveTable> AddCombination){
			int num=0;
			//得到session
		...
		public int insertOne(serveTable ET){
			int num=0;
			//得到session
		...
		public void deleteContent(String name){
			...
			try{
				...
				String hql="delete from serveTable e where e.name=?";
		...
		public void deleteCombinationAById(int ID){
			...
			try{
				...
				transaction=session.beginTransaction();
				serveTable SC=(serveTable) session.load(serveTable.class,ID);
				session.delete(SC);
