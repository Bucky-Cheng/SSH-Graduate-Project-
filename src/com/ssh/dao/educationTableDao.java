...
public class educationTableDao {

	@Autowired
	SessionFactory sessionFactory;
	
	public educationTableDao(){
	...
	public void setSessionFactory(SessionFactory sessionFactory) {
	 ...
	 public boolean updateEduTable(String name,String content){
			...
			try{
				...
				session=sessionFactory.openSession();
				//根据id获取要修改的用户数据
				String hql=("update educationTable e set e.content=? where name =?");  
				Query sqlquery = session.createQuery(hql);
	 ...
	 public List<educationTable> getTableByName(String Name){
		 Session session=null;
		 try{
			 session=sessionFactory.openSession();
			 String hql="from serveTable where name='"+Name+"'";
			 Query query=session.createQuery(hql);
			 List<educationTable> list=query.list();
			 System.out.println(list.get(0).getName());
			 if(list.size()>0){
	 ...
	 public boolean updateEduScore(String name,String Score){
			...
			try{
				...
				session=sessionFactory.openSession();
				//根据id获取要修改的用户数据
				String hql=("update educationTable e set e.score=? where name =?");  
				Query sqlquery = session.createQuery(hql);
	 ...
	 public boolean updateEduScoreA(String name,String Score){
			...
			try{
				...
				session=sessionFactory.openSession();
				//根据id获取要修改的用户数据
				String hql=("update educationTable e set e.AllScore=? where name =?");  
				Query sqlquery = session.createQuery(hql);
		...
		public int insertOneB(List<educationTable> AddCombination){
			int num=0;
			//得到session
		...
		public int insertOne(educationTable ET){
			int num=0;
			//得到session
		...
		public void deleteContent(String name){
			...
			try{
				...
				String hql="delete from educationTable e where e.name=?";
		...
		public void deleteCombinationAById(int ID){
			...
			try{
				...
				transaction=session.beginTransaction();
				educationTable SC=(educationTable) session.load(educationTable.class,ID);
				session.delete(SC);
